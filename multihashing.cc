...
static defyx_cache* rx_cache = nullptr;
static defyx_vm* rx_vm = nullptr;
...
void init_rx(const uint8_t* seed_hash_data) {
    if (!rx_cache) {
        rx_cache = defyx_alloc_cache(static_cast<defyx_flags>(RANDOMX_FLAG_JIT | RANDOMX_FLAG_LARGE_PAGES));
        if (!rx_cache) {
            rx_cache = defyx_alloc_cache(RANDOMX_FLAG_JIT);
        }
        memcpy(rx_seed_hash, seed_hash_data, sizeof(rx_seed_hash));
        defyx_init_cache(rx_cache, rx_seed_hash, sizeof(rx_seed_hash));
        if (rx_vm) {
            defyx_vm_set_cache(rx_vm, rx_cache);
        }
    }
    else {
    ...
    if (!rx_vm) {
        ...
        rx_vm = defyx_create_vm(static_cast<defyx_flags>(flags), rx_cache, nullptr);
        if (!rx_vm) {
            rx_vm = defyx_create_vm(static_cast<defyx_flags>(flags - RANDOMX_FLAG_LARGE_PAGES), rx_cache, nullptr);
        }
    }
}
...
NAN_METHOD(randomx) {
    ...
    char output[32];
    defyx_calculate_hash(rx_vm, reinterpret_cast<const uint8_t*>(Buffer::Data(target)), Buffer::Length(target), reinterpret_cast<uint8_t*>(output));
